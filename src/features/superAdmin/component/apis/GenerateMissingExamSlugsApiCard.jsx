...
import { useGenerateMissingExamSlugs } from '../../hooks';
...
export const GenerateMissingExamSlugsApiCard = () => {
    const {
        loading,
        error,
        result,
    ...
    const handleExecute = async () => {
        await generateMissingExamSlugs({});
    };
    ...
    const resultColumns = [
        ...
        },
        {
            ...
            label: 'Title',
            render: (row) => (
                <span className="text-sm text-foreground-light max-w-[220px] truncate block" title={row?.title || row?.name || row?.examTitle || row?.exam?.title || ''}>
                    {row?.title || row?.name || row?.examTitle || row?.exam?.title || '-'}
                </span>
            ),
        },
        {
            ...
            render: (row) => (
                <span className="text-xs text-foreground-light">
                    {row?.slug || row?.generatedSlug || row?.exam?.slug || '-'}
                </span>
            ),
        },
        {
                    ...
                    <>
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                            ...
                            <div className="rounded-sm border border-border p-3">
                                ...
                                <p className="text-lg font-semibold text-foreground">{Array.isArray(responseResults) ? responseResults.length : 0}</p>
                            </div>
                        </div>
