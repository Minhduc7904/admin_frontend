...
export const MyAssistantShiftCard = ({ shift, compact = false }) => {
  ...
  return <article className={`rounded-lg border p-2 shadow-sm ${statusStyle[status] || statusStyle.PENDING}`}><div className="flex items-start justify-between gap-1"><p title={shift.name} className={`min-w-0 font-semibold ${compact ? 'line-clamp-1 text-[11px]' : 'line-clamp-2 text-sm leading-snug'}`}>{shift.name}</p>{shift.isLocked && <LockKeyhole className="mt-0.5 h-3 w-3 shrink-0 text-amber-700" />}</div><p className={`mt-1 font-semibold opacity-80 ${compact ? 'text-xs' : 'text-sm'}`}>{timeLabel(shift.startAt)} – {timeLabel(shift.endAt)}</p>{!compact && <p title={shift.courseClass?.name || shift.courseClass?.className || shift.series?.name} className="mt-1 line-clamp-2 text-xs opacity-75">{shift.courseClass?.name || shift.courseClass?.className || shift.series?.name}</p>}{shift.notes && <div className={`mt-2 rounded-md bg-white/65 px-2 py-1.5 font-bold leading-snug ${compact ? 'line-clamp-2 text-[10px]' : 'line-clamp-3 text-xs'}`}><span className="mr-1">Ghi chú:</span>{shift.notes}</div>}</article>;
};
