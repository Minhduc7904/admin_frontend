...
const createLayout = (items) => {
  ...
  sorted.forEach((item) => { const start = minuteOf(item.startAt); let lane = ending.findIndex((end) => end <= start); if (lane < 0) lane = ending.length; ending[lane] = minuteOf(item.endAt); output[item.assistantShiftId] = lane; });
...
const createScale = (shifts) => {
  ...
  shifts.forEach((shift) => {
    ...
    coverage.forEach((value, index) => { if (value) heights[index] += extra * value / total; });
  });
...
const RegistrationShiftCard = ({ shift, profile, now, actionShiftId, pendingActionShiftIds, canRegister, canCancel, canTransfer, canSwap, onRegister, onCancel, onTransfer, onSwap }) => {
  ...
  const persistentLockReason = pendingActionShiftIds.includes(shift.assistantShiftId) ? 'Chờ xác nhận'
    : !isPast && (
  ...
  const palette = persistentLockReason
                  ...
                  return (
                    <div
                      ...
                      style={{
                        ...
                        height: Math.max(requiredCardHeight(shift), scale.position(end) - scale.position(start)),
